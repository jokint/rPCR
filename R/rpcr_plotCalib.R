...
rpcr_plotCalib <- function(data = NULL,
  ...
  if (!is.null(data)) {
    ...
    plot_list[[1]] <-plot_list[[1]] +
      geom_point(data = df, aes(x = tprop, y = get(var), col = target)) +
      labs(
        x = "tprop",
        y = signal,
    ...
    plot_list[[2]] <-plot_list[[2]] +
      geom_point(data = df, aes(x = tprop, y = get(var) - tprop, col = target)) +
      labs(
        x = "tprop",
    ...
    plot_list
    if (smooth) {
      plot_list[[1]] <-plot_list[[1]] + geom_line(data = df, stat = "smooth", method = "loess", aes(x = tprop, y = get(var), col = target),linewidth=1.5,alpha=0.5)
      plot_list[[2]] <-plot_list[[2]] + geom_line(data = df, stat = "smooth", method = "loess", aes(x = tprop, y = get(var) - tprop, col = target),linewidth=1.5, alpha = 0.5)
    }

    if (error) {
      df1 <- aggregate(. ~ tprop + target, df[, c("tprop", "target",  var)], function(x) c(mean = mean(x), sd = sd(x)))
      df1 <- cbind(df1[1:2],df1[[3]])
      df2 <- aggregate(. ~ tprop + target, df[, c("tprop", "target", "delta")], function(x) c(mean = mean(x), sd = sd(x)))
      df2 <- cbind(df2[1:2],df2[[3]])
      plot_list[[1]] <-plot_list[[1]] + geom_errorbar(data = df1, aes(x = tprop, ymin = mean - sd, ymax = mean + sd, col = target), position = position_dodge(width = 1/percent))
      plot_list[[2]] <-plot_list[[2]] + geom_errorbar(data = df2, aes(x = tprop, ymin = mean - sd, ymax = mean + sd, col = target), position = position_dodge(width = 1/percent))
    }

    if (confidence) {
      plot_list[[1]] <-plot_list[[1]] + geom_smooth(data = df, aes(x = tprop, y = get(var), fill = target), col = NA, method = "loess")
      plot_list[[2]] <-plot_list[[2]]  + geom_smooth(data = df, aes(x = tprop, y = get(var) - tprop, fill = target), col = NA, method = "loess")
    }
