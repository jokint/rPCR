...
df0$FluoAdelta <- df0$FluoA - para$bFluoA + rnorm(length(df0$FluoA), mean = 0, sd = 0.1)
# df0$FluoAdelta <- if (df0$FluoAdelta > (0.01 * max(df0FluoAdelta)) df0$FluoAdelta
...
plots[[4]] <- ggplot() +
  ...
  geom_line(data = df0,
            aes(x = cycle,
                y = (FluoA-min(FluoA))/(max(FluoA)-min(FluoA)),
                # col = par_startRatio,
                group = well),
