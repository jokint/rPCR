...
results <- readRDS(paste0(mPath,path_sim,"Sim_",title,"/results/results_small.rds"))
...
ppp1 <- lapply(signals,function(sig){
  df <- df0
  df[df[,sig]<1e-22,sig] <-  0
...
plots[[2]] <-  basic_plot +
  geom_line(data = df0,
            aes(x = cycle,
                ...
                group = well),
            linewidth = wline)  +
  labs(x ="cycle", y = "unquenched Fluorsence molecules")
...
plots[[5]] <- plots[[1]] +
  annotation_custom(
    ggplotGrob(plots[[1]]+ scale_y_continuous(trans = "log10")) ,
    xmin = inset[1],
    xmax = inset[2],
    ymin = min(df0[,sig ]) + inset[3]*(max(df0[,sig ])-min(df0[,sig ])),
    ymax = min(df0[,sig ]) + inset[4]*(max(df0[,sig ])-min(df0[,sig ]))
  )
...
plots[[6]] <- plots[[2]] +
  annotation_custom(
    ggplotGrob(
      plots[[6]] <- basic_plot +
        geom_line(data = df0[df0$par_startRatio!=0,],
                  aes(x = cycle,
                      y = signalA.c,
                      col = par_startRatio,
                      group = well),
                  linewidth = wline)  +
        labs(x ="cycle", y = "log(released dye") + theme_rpcr() + scale_colour_gradientn(colours = rpcr_col_probe) +
...
plots[[7]] <- plots[[3]] +
  annotation_custom(
    ggplotGrob(plots[[3]]+ scale_y_continuous(trans = "log10")) ,
    xmin = inset[1],
    xmax = inset[2],
    ymin = min(df0[,sig ]) + inset[3]*(max(df0[,sig ])-min(df0[,sig ])),
    ymax = min(df0[,sig ]) + inset[4]*(max(df0[,sig ])-min(df0[,sig ]))
...
plots[[8]] <- plots[[4]] +
  annotation_custom(
    ggplotGrob(plots[[4]]+ scale_y_continuous(trans = "log10")) ,
    xmin = inset[1],
    xmax = inset[2],
    ymin = min(df0[,sig ]) + inset[3]*(max(df0[,sig ])-min(df0[,sig ])),
    ymax = min(df0[,sig ]) + inset[4]*(max(df0[,sig ])-min(df0[,sig ]))
...
plots[[4]] <- ggplot() +
  geom_line(data = df0,
            aes(x = cycle,
                ...
                col = par_startRatio,
                group = well),
            linewidth = wline,linetype=tline)  +
  labs(x ="cycle", y = "signal") + theme_rpcr() + scale_colour_gradientn(colours = rpcr_col_probe)
