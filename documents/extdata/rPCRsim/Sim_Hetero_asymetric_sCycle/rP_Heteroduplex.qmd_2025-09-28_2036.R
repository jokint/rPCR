...
results$effA <- results$difA/results$sA * 100
results$effB <- results$difB/results$sB * 100
results$effT <- results$difT/results$sT * 100
...
```{r}
ggplot(data=results) +geom_line(aes(x=par_startRatio,y=effA,col=factor(sT),group=interaction(par_hetero_k1,par_startTemplate)))
...
res <- lapply(unique(sel$sT)[-1], function(template){
  ...
  # df$effT <- (df$effT-min(df$effT))/100
  # df <- df[!(df$sprop %in% c(0,1)),]
  # fit <- nlsLM(effT ~  ratioeff(sratio,a,b,c),data=df,start=list(a=1,b=1,c=1))
  # fit <- nlsLM(prop ~  propdrift(sprop,g),data=df,start=list(g=0.5))
  #fit <- nlsLM(ratio ~  g * slratio,data=df,start=list(g=0.5))
  # fit <- nlsLM(prop ~  propdrift(sprop,g)*(1-propeff(sprop,a,b,c)),data=df,start=list(g=0.5,a=0.95,b=0.95,c=0.5))
  # summary(fit)
  fit <- nlsLM(prop ~ drift(sprop,a,b,c),data=df,start=list(a=1,b=1,c=1))
  plot(x=df$sprop,y=residuals(fit))
  summary(fit)
})

sel <- results[results$par_hetero_k1==3e5&results$sT==unique(results$sT)[3],]
ggplot() +
  geom_line(data=sel,aes(x=par_startRatio,y=effA,group=interaction(par_hetero_k1,par_startTemplate)),col=3) +
  geom_line(data=sel,aes(x=par_startRatio,y=effB,group=interaction(par_hetero_k1,par_startTemplate)),col=4) +
  geom_line(data=sel,aes(x=par_startRatio,y=effT,group=interaction(par_hetero_k1,par_startTemplate)),col=1) +
